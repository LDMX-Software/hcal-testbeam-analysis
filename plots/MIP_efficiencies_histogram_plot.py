...
# the data file in which the histogram is stored (MIP_efficiencies_hist.csv)
...
if __name__ == '__main__':
    ...
    # Format data
    filtered_data = hist_data['filtered_data'].to_numpy().flatten()
    MC_data = hist_data['MC'].to_numpy().flatten()
    unfiltered_data = hist_data['unfiltered_data'].to_numpy().flatten()
    bins = hist_data['bins'].to_numpy().flatten()

    # Plot data
    n, bins1, patches = plt.hist(bins, len(bins), weights=filtered_data, histtype='stepfilled', linewidth=2, label="Data (Filtered)", color=dataColor)
    n2, bins2, patches2 = plt.hist(bins, len(bins), weights=MC_data, histtype='step', linewidth=2, label="MC", color=mcColor)
    n3, bins3, patches3 = plt.hist(bins, len(bins), weights=unfiltered_data, histtype='step', linewidth=2, label="Data (unfiltered)", color=unfilColor)
